        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-5">
                        <div class="col-12">
                            <Button class="btn btn-success text-white btn-modal-new-form" target-modal="#smppUserForm" target-url="{{ route('smpps.users.create')}}">Add New</Button>
                        </div>
                    </div>
                   <table class="table">
                    <thead>
                        <tr>
                            <th>Division</th><th>Client</th><th>service_type</th><th>Batchname</th><th>Created</th><th>Modified</th><th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td>{{$user['division']}}</td><td>{{$user['client']['name']}}</td><td>{{$user['service_type']}}</td> <td>{{$user['batchname']}}</td> <td><?=$user['date_created'];?></td><td><?=$user['date_modified'];?></td><td><a href="{{route('smpps.users.edit',['user'=>$user['client_id']])}}" target-modal="#smppUserForm" class="btn btn-warning btn-sm text-white update-modal-form">Edit</a> <a href="{{route('smpps.users.delete',['user'=>$user['client_id']])}}" class="btn btn-danger btn-sm text-white ajax-delete">Delete</a> </td>
                        </tr>
                        @endforeach
